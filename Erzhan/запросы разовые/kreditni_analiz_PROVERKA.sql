...
[TIP_DOKUMENTA]
	  ...
	  ,CAST(DATEDIFF_BIG(ms, '1970-01-01', CAST(DATA_OPLATY_VOZVRATA AS DATE)) AS BIGINT) AS UnixTimestamp
      ,[NOMER_DOGOVORA]
      ,(-1)*[SUMMA] AS [SUMMA]
      ,[ORGANIZATSYA_GUID]
      ,[KONTRAGENT_GUID]
...
AND DOGOVOR_GUID IN (0x80B900155D01C90111E73EBBE8BF5218)
)
...
,DOGOVOR.SUMMA AS SUMMA_DOOVORA
,SUMMA_ZADOLZHNOSTY 
,PROSROSHKA.SUMMA AS SUMMA_PROROCHKI
FROM DOGOVOR
INNER JOIN (SELECT ZADOLZNOST.SUMMA AS SUMMA_ZADOLZHNOSTY , ZADOLZNOST.DOGOVOR_GUID
				FROM ZADOLZNOST
				INNER JOIN (SELECT MAX(ZADOLZNOST.TIP) AS TIP	, DOGOVOR_GUID
								FROM ZADOLZNOST
								WHERE TIP LIKE 'GRAFIK_'
								GROUP BY DOGOVOR_GUID
							) A	ON A.DOGOVOR_GUID=ZADOLZNOST.DOGOVOR_GUID
								AND A.TIP=ZADOLZNOST.TIP
			) AS ZADOLZNOST ON ZADOLZNOST.DOGOVOR_GUID=DOGOVOR.GUID

LEFT JOIN (SELECT ZADOLZNOST_NA_SEGODNYA.DOGOVOR_GUID , ZADOLZNOST_NA_SEGODNYA.SUMMA
