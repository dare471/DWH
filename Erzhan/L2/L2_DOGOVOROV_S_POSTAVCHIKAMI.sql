...
SELECT MAX([PERIOD]) AS DATE 
      ,[NOMENKLATURA_GUID]
      ,[DOGOVOR_GUID]
      ,SUM([KOLICHESTVO]*[SEBESTOIMOST])SUMMA
...
,SPEKA AS (SELECT [DOGOVOR_GUID]
  ...
  LEFT JOIN [L1].[dbo].DOGOVORY_KONTRAGENTOV AS DK
	...
	WHERE DK.TIP_DOGOVORA='С поставщиком / исполнителем'
GROUP BY 
	[DOGOVOR_GUID]
	,[PROSROCHENNYY_TOVAR]
    ,[NOMENKLATURA_GUID]
...
WHERE [synonym_registrator] in ('Списание безналичных денежных средств','Расходный кассовый ордер')
...
SELECT DISTINCT SPD.[DOGOVOR_GUID]
	...
	,O.SUMMA AS SUMMA_OTGRUZKA
	,V.DATE AS DATE_VOZVRAT
	,V.KOLICHESTVO AS KOLICHESTVO_VOZVRAT
	,V.SUMMA_VYRUCHKI AS SUMMA_VYRUCHKI
